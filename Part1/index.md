...
1. 运行网络调试助手，风络设置：协议类型TCP Client、本地主机地址： __你电脑的内网IP、远程主机地址内网IP__:端口。点击连接。
...
- `AT+CIPSTART="TCP","${HOST}",${PORT}` 填入你的内网IP地址与端口并发送，建立TCP连接，像我就是`AT+CIPSTART="TCP","192.168.1.121",2424` ，大家肯定跟我不一样的，要自己换。
